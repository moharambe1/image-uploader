...
const UploadImg = (props) => {
  ...
  useEffect(() => {
    ...
    setInterval(()=>{console.log(DragItemsConter);},2000)
  ...
  return (
    <UploadDiv  ref={dropRef}>
      ...
      <div className={`${classCss.drop_img_dev} ${isDrag? classCss.highlight:null}`}>
        <p className={isDrag? classCss.text_highlight:null}> {isDrag?"Drop Here":"Drag & Drop your image here"}</p>
      </div>
  ...
  function handleDrag(e) {
    ...
    if(!isDrag && e.dataTransfer.items.length>0)
      setIsDrag(true);
  }
    ...
    if(e.dataTransfer.files && e.dataTransfer.files.length>0){
      ...
      }catch(err)
      {
        if(props.handleDrop==null)
          console.error("Error: mast provided handleDrop function to handle doped files")
        console.error(err);
      }
      DragItemsConter=0;
    }
